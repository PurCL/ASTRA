...
@default_subscription
class TaskIntentionReviewAgent(RoutedAgent):

    def __init__(
        self,
        description: str,
        reasoning_sampler: ReasoningSampler,
        review_prompt_fname: str = "agent/sec_event_composer/prompts/intention_review.txt",
        enable_diversity: bool = False,
        existing_tasks: List[str] = [],
    ):
        super().__init__(description=description)
        self._name = "IntentionReviewAgent"
...
""".format(
    ...
    async def handle_textual_task_review_request(
        self,
        message: IntentionReviewRequest,
        ...
        if self._enable_diversity:
            ...
            for tag, async_task in tag2diversity_task.items():
                ...
                result_entry = IntentionReviewResultEntry(
        ...
        if len(promising_tasks) == 0:
            review_result = IntentionReviewResult(
